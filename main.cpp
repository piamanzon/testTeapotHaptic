...
GLuint sphereId = 0; // id of sphere
bool mouseLeftDown;
bool mouseRightDown;
float mouseX, mouseY;
                                   ...
                                   HLcache *cache, void *userdata)
{
    ...
    else if (object == sphereId) {
        
        if (isResize) {
            isResize = false;
            std::cout << "INSIDE IFFFFF" << std::endl;
...
int main(int argc, char **argv)
{
    ...
    sphereId = hlGenShapes(7);

    // creating ID for sphere
...
void initHL()
{
  ...
  //  hlAddEventCallback(HL_EVENT_TOUCH, sphereId, HL_CLIENT_THREAD,
    //    &touchShapeCallback, NULL);
...
{    
    ...
    hlBeginShape(HL_SHAPE_FEEDBACK_BUFFER, sphereId);
    drawSphere(isResize);
